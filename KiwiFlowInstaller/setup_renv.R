...
tryCatch({
    if (!requireNamespace("renv", quietly = TRUE)) {
        stop("ERROR: 'renv' package not available for restore operation. Please ensure it was installed.")
    }
...
if (!overall_success) {
    ...
    message("R script 'setup_renv.R' completed successfully. Exiting R with success status (0).")
    q(save = "no", status = 0)
}