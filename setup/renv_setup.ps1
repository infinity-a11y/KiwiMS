...
try {
    ...
    if ($condaRunWrapperOutput.Trim() -ne "") {
        Write-Host "--- Conda Run Wrapper Output (if any) ---"
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        # Now, read and log the content of the .Rout file for detailed R errors
        if (Test-Path $routFile) {
        ...
        throw "R environment restoration failed. Check log for details and the .Rout file content."
    }
    ...
    Write-Host "R package installation and renv setup completed successfully."

}
catch {
    Write-Host "FATAL ERROR: Failed to restore R environment using script '$rScriptPath'."
